 ...
 * 	@UpdatedOn	: 03-26-2026 - fix: replace removed findByReadingDateAndReadingSlotIdAndStructureId
 *                              with findByPipelineIdAndReadingDate per pipeline
 *
 * 	@Type		: Class
 * 	@Layer		: Service
...
import dz.sh.trc.hyflo.intelligence.dto.analytics.PipelineCoverageDTO;
...
public class SlotCoverageService {
     ...
     *
     * Loads all pipelines for the structure, then fetches readings for each
     * pipeline on the given date. Slot filtering is done in-memory by
     * matching readingSlot.id (FlowReading.readingSlot FK added in fix commit).
     */
    public SlotCoverageResponseDTO getSlotCoverage(LocalDate date, Integer slotNumber, Long structureId) {
        log.info("Getting slot coverage for date={}, slot={}, structure={}",
        ...
        // Load readings per pipeline for the given date, then keep only
        // the one matching the requested slot (if any).
        // This replaces the removed findByReadingDateAndReadingSlotIdAndStructureId.
        Map<Long, FlowReading> readingMap = pipelines.stream()
                .flatMap(pipeline -> flowReadingRepository
                        .findByPipelineIdAndReadingDate(pipeline.getId(), date)
                        .stream()
                        .filter(r -> r.getReadingSlot() != null
                                && slotNumber.longValue() == r.getReadingSlot().getId()))
                .collect(Collectors.toMap(
                        r -> r.getPipeline().getId(),
                        r -> r,
                        (existing, replacement) -> replacement));

        List<PipelineCoverageDTO> pipelineCoverage = pipelines.stream()
                .map(pipeline -> buildPipelineCoverageItem(
                        pipeline, readingMap.get(pipeline.getId())))
                .collect(Collectors.toList());

        int totalPipelines     = pipelineCoverage.size();
        int recordedCount      = (int) pipelineCoverage.stream()
                .filter(p -> p.getReadingId() != null).count();
        int submittedCount     = (int) pipelineCoverage.stream()
                .filter(p -> p.getValidationStatus() != null
                        && "SUBMITTED".equals(p.getValidationStatus().getCode())).count();
        int approvedCount      = (int) pipelineCoverage.stream()
                .filter(p -> p.getValidationStatus() != null
                        && "APPROVED".equals(p.getValidationStatus().getCode())).count();
        int rejectedCount      = (int) pipelineCoverage.stream()
                .filter(p -> p.getValidationStatus() != null
                        && "REJECTED".equals(p.getValidationStatus().getCode())).count();
        int missingCount       = totalPipelines - recordedCount;

        double recordingCompletion   = totalPipelines > 0
                ? (recordedCount * 100.0 / totalPipelines) : 0.0;
        double validationCompletion  = totalPipelines > 0
                ? ((approvedCount + rejectedCount) * 100.0 / totalPipelines) : 0.0;
    ...
    private PipelineCoverageDTO buildPipelineCoverageItem(Pipeline pipeline, FlowReading reading) {
        ...
        if (reading != null) {
            String statusCode = reading.getValidationStatus() != null
                    ? reading.getValidationStatus().getCode() : "DRAFT";
            builder
    ...
    private ValidationStatusDTO convertValidationStatusToDTO(
            dz.sh.trc.hyflo.flow.common.model.ValidationStatus status) {
    ...
    private EmployeeDTO convertEmployeeToDTO(
            dz.sh.trc.hyflo.general.organization.model.Employee employee) {
